...
resource "azurerm_private_endpoint" "test_link" {
  name                = "pe-test-${lower(var.domain_name_short)}-${lower(var.environment_short)}-${lower(var.environment_instance)}"
  resource_group_name = azurerm_resource_group.this.name
  location            = azurerm_resource_group.this.location
  subnet_id           = azurerm_subnet.this.id

  private_service_connection {
    name                           = "func-testprivateconnection"
    private_connection_resource_id = module.func_test.id
